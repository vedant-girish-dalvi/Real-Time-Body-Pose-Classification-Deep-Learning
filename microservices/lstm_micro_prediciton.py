...
consumer = KafkaConsumer(
    ...
    group_id='48', # Specify the consumer group ID
...
MODEL_FILE = fr"{current_work_dir}/96.32_LSTM.h5"
...
def predict(test_data):
    ...
    # predict_thread.join()
    end_time = time.time()
    save_to_influx("micro_lstm", np.ndarray.item(results), end_time - start_time, predict_end - predict_start, cpu_usage, memory_usage)
    # producer.send("microlstm", value=np.ndarray.item(results))
    # print(end_time)
    # return results
...
def main():
    for message in consumer:
        if message != "":
            ...
            array = np.array(values, dtype=float) #check
